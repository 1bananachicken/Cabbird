...
#include <string_view>
#include <type_traits>
namespace cabbird::sdk {
[[nodiscard]] constexpr CabbirdStringViewV1 StringView(std::string_view value) noexcept { return {value.data(), value.size()}; }
[[nodiscard]] constexpr CabbirdStatusV1 Ok() noexcept { return {CABBIRD_STATUS_V1_OK, 0, {nullptr, 0}}; }
[[nodiscard]] constexpr bool Succeeded(const CabbirdStatusV1& status) noexcept { return status.code == CABBIRD_STATUS_V1_OK; }
...
class Host final {
public:
    explicit constexpr Host(const CabbirdHostApiV1* host) noexcept : host_(host) {}
    template <typename Service> [[nodiscard]] ServiceRef<Service> Query(std::string_view id, std::uint32_t version = 1) const noexcept {
        ...
        constexpr std::size_t host_query_size =
            offsetof(CabbirdHostApiV1, query_service) +
            sizeof(decltype(CabbirdHostApiV1::query_service));
        if (host_ == nullptr || host_->struct_size < host_query_size ||
            host_->api_major != CABBIRD_PLUGIN_API_V1_MAJOR || host_->query_service == nullptr) return {};
        const void* table{};
...
private: const CabbirdHostApiV1* host_{};
};
class UiWindow final {
public:
    UiWindow(const CabbirdUiServiceV1* ui, std::string_view title, int* open = nullptr, std::uint32_t flags = 0) noexcept
...
private: const CabbirdUiServiceV1* ui_{}; bool active_{};
};
}  // namespace cabbird::sdk
