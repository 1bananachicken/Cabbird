...
#include "cabbird/plugin_catalog.hpp"
...
struct InstalledPluginView final : unitymem::PluginView {
    InstalledPluginView() = default;
    explicit InstalledPluginView(unitymem::PluginView runtime)
        : unitymem::PluginView(std::move(runtime)), has_runtime_view(true) {}
...
enum class UnityCompatibilityLevel : std::uint8_t {
    Unknown,
    CoreOnly,
    Partial,
    Supported,
...
// One method a Unity profile document declares, with the host's binding result for it.
struct UnityProfileMethodStatus {
    std::string id;
    bool bound{};
    std::string how;  // "metadata" / "profile", or why no binding happened
};

/* The Unity profile document, as the compatibility page needs it.
 *
 * WHY THIS IS NOT A BuildProfile.  Upstream's matrix is one row per declared FEATURE, where a
 * feature is a set of byte-pattern SYMBOLS resolved by a scan.  Cabbird's Unity target does not
 * resolve anything that way: it resolves named IL2CPP methods through the runtime's own
 * metadata (see unity_build_profile.hpp).  So the honest matrix for this target is one row per
 * method the profile declares, with the binding result as the availability column -- and the
 * profile source/hash come from that same document instead of from a BuildProfile file this
 * tree does not ship.
 */
struct UnityProfileDocumentStatus {
    bool loaded{};
    std::string build_id;
    std::string game;
    std::string assembly;
    std::string source;  // the document's path
    std::string hash;    // SHA-256 of the document as read
    std::string error;   // why it is not loaded
    std::vector<UnityProfileMethodStatus> methods;
};
...
struct UnityCompatibilitySnapshot {
    std::string game_id;
    std::string build_id;
    std::string profile_source;
    std::string profile_hash;
    UnityCompatibilityLevel level{UnityCompatibilityLevel::Unknown};
    std::string reason;
    std::vector<UnityFeatureSnapshot> features;
    std::vector<AvailableServiceVersion> services;

    // Holds `Supported` down unless every declared row is available AND validated, so a
    // level can never be greener than the rows it is describing.  An empty matrix is
    // never Supported.
    void NormalizeLevel() noexcept;
    [[nodiscard]] const UnityFeatureSnapshot* FindFeature(std::string_view id) const noexcept;
};
...
struct PlatformUiSnapshot final {
    ...
    UnityCompatibilitySnapshot unity_compatibility;
...
/* The compatibility snapshot: level, build id, profile source and hash all come from the Unity
 * profile document the frame clock binds against, and every matrix row is a method that document
 * declares.  A host with no document (or a broken one) reports `Unknown` and says why -- it does
 * not fall back to a second profile format, because this tree ships only one. */
[[nodiscard]] UnityCompatibilitySnapshot BuildUnityCompatibilitySnapshot(
