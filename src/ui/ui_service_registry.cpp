...
bool UiServiceRegistry::Publish(const CabbirdUiServiceV1* service) noexcept {
    if (service == nullptr || service->struct_size < sizeof(CabbirdUiServiceV1) ||
...
void UiServiceRegistry::Withdraw(const CabbirdUiServiceV1* service) noexcept {
    {
        std::scoped_lock lock(mutex_);
