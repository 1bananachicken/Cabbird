...
constexpr std::wstring_view kAnalyzerConfigPath = L"cabbird.ini";
...
nlohmann::json SerializeColor(const CabbirdUiColor& color) {
    return nlohmann::json::array({color.red, color.green, color.blue});
}

void ReadColorValue(
    const nlohmann::json& values, const char* id, CabbirdUiColor& output) {
...
bool IsValidColor(const CabbirdUiColor& color) noexcept {
...
nlohmann::json Serialize(
    ...
    return {
        ...
        {"values", {
            {"interface.palette", unitymem::ToString(values.interface_palette)},
...
std::vector<PlatformSettingsValidationError> ValidatePlatformSettings(
    ...
    const auto validate_color = [&](const char* id, const CabbirdUiColor& color) {
...
public:
    ...
    bool Start() {
        ...
        try {
            ...
            snapshot_.values.interface_palette = unitymem::ParseCabbirdUiPalette(
                values.value("interface.palette", std::string{"cabbirdhub"}));
