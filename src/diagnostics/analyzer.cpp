...
constexpr std::size_t kMaximumHeapScanBytes = 2U * 1024U * 1024U * 1024U;
constexpr std::size_t kHeapScanChunkBytes = 1024U * 1024U;
...
std::vector<std::uint8_t> EncodeScanNeedle(
    std::string_view encoding, std::string_view value) {
    if (encoding == "ascii") {
...
bool AddressIntersectsPluginModule(
    const cabbird::CoreMemoryServices& services,
...
bool IsPluginModule(
    const unitymem::ModuleInfo& module, const std::filesystem::path& root,
    const AnalyzerConfig& config) {
    const std::wstring plugin_root = AbsolutePluginRoot(config, root).wstring();
...
std::string MutationJson(
    const cabbird::ModuleMemoryService& memory,
...
Analyzer::Analyzer(
    ...
    : root_(std::move(root)),
      ...
      unity_reflection_query_(std::move(unity_reflection_query)) {}
...
std::string Analyzer::HeapScanJson(
    std::string_view encoding,
    std::string_view needle,
    const std::size_t limit) const {
    if (!memory_services_.memory) return Error("memory service unavailable");
    if (needle.empty() || needle.size() > 4096 || limit == 0 || limit > 4096)
        return Error("usage: heap-scan <ascii|utf16> <text> [limit<=4096]");
    const auto pattern = EncodeScanNeedle(encoding, needle);
    if (pattern.empty()) return Error("encoding must be ascii or utf16");

    const auto readable = [](const DWORD protection) {
        const auto base = protection & 0xff;
        return base != PAGE_NOACCESS && base != PAGE_EXECUTE &&
            base != PAGE_GUARD;
    };
    const auto regions = memory_services_.memory->EnumerateProcessRegions();
    ...
    for (const auto& region : regions) {
        if (region.state != MEM_COMMIT || region.type != MEM_PRIVATE ||
            !readable(region.protection) || region.size == 0) continue;
        if (scanned >= kMaximumHeapScanBytes) break;
        const auto region_size = (std::min)(region.size, kMaximumHeapScanBytes - scanned);
        std::vector<std::uint8_t> carry;
        std::size_t offset = 0;
        while (offset < region_size && hits.size() < limit) {
            const auto count = (std::min)(kHeapScanChunkBytes, region_size - offset);
            const auto bytes = memory_services_.memory->ReadMemory(region.base + offset, count);
            if (!bytes) {
                offset += count;
                continue;
            }
            ...
            const auto base = region.base + offset - carry.size();
            for (std::size_t pos = 0;
                 pos + pattern.size() <= window.size() && hits.size() < limit; ++pos) {
                if (std::equal(pattern.begin(), pattern.end(), window.begin() + pos))
                    hits.push_back(base + pos);
            }
            carry.assign(
                window.end() - (std::min)(overlap, window.size()), window.end());
            offset += count;
        }
        scanned += region_size;
        ++regions_scanned;
    }
    std::string result = "\"encoding\":" + json::Quote(encoding) +
        ",\"needle\":" + json::Quote(needle) +
        ",\"hits\":[";
    for (std::size_t i = 0; i < hits.size(); ++i) {
        if (i) result += ',';
        result += json::Hex(hits[i]);
    }
    result += "],\"hit_count\":" + std::to_string(hits.size()) +
        ",\"regions_scanned\":" + std::to_string(regions_scanned) +
        ",\"bytes_scanned\":" + std::to_string(scanned) +
        ",\"truncated\":" + (hits.size() >= limit ? "true" : "false");
    return Ok(result);
}
...
std::string Analyzer::Execute(std::string_view command_line) const {
    ...
    if (command == "help") {
        return Ok("\"commands\":[\"ping\",\"status\",\"modules\",\"sections <module|.>\",\"regions <module|.>\",\"scan <module|.> <section> <pattern>\",\"heap-scan <ascii|utf16> <text> [limit<=4096]\",\"il2cpp-objects <namespace|-> <class> [field|-] [limit<=256]\",\"xrefs <module|.> <target>\",\"ue\",\"ue actors <filter|*> [limit] [cursor]\",\"ue objects <filter|*> [limit] [cursor]\",\"ue functions <filter|*> [limit] [cursor]\",\"ue fname <id>\",\"ue ftext <address>\",\"ue combat\",\"ue buffs\",\"read <address> <size<=1048576>\",\"write <address> <hex bytes>\",\"patch <address> <hex bytes>\",\"protect <address> <size> <r|rw|x|rx|rwx>\",\"alloc <size> [protection]\",\"free <address>\",\"chain <base> [offset ...]\",\"rip <instruction> <disp_offset> <instruction_size>\",\"ptr <address>\",\"f32 <address> [count]\",\"f64 <address> [count]\",\"snapshot [filename]\"]");
    }
    if (command == "status") {
    ...
    if (command == "ue") {
        ...
        if (!unity_reflection_query_) return Error("UE reflection queries are unavailable");
        try {
            return unity_reflection_query_(arguments);
        } catch (...) {
    ...
    if (command == "heap-scan") {
        const auto [encoding, after_encoding] = Shift(arguments);
        const auto [needle, limit_text] = Shift(after_encoding);
        const auto limit = limit_text.empty() ? std::optional<std::uint64_t>(256) :
            ParseInteger(limit_text);
        if (encoding.empty() || needle.empty() || !limit)
            return Error("usage: heap-scan <ascii|utf16> <text> [limit<=4096]");
        return HeapScanJson(encoding, needle, static_cast<std::size_t>(*limit));
    }
    if (command == "il2cpp-objects") {
        const auto [namespaze, after_namespace] = Shift(arguments);
        const auto [class_name, after_class] = Shift(after_namespace);
        const auto [field_name, limit_text] = Shift(after_class);
        const auto limit = limit_text.empty() ? std::optional<std::uint64_t>(32) :
            ParseInteger(limit_text);
        if (namespaze.empty() || class_name.empty() || !limit)
            return Error("usage: il2cpp-objects <namespace> <class> [field] [limit<=256]");
        return cabbird::RequestUnityIl2CppObjectScan(
            namespaze == "-" ? std::string_view{} : namespaze, class_name,
            field_name == "-" ? std::string_view{} : field_name, static_cast<std::size_t>(*limit));
    }
