...
std::string SerializeServiceGraphSnapshotJson(const ServiceGraphSnapshot& snapshot) {
    ...
    for (std::size_t index = 0; index < snapshot.failures.size(); ++index) {
        ...
        output += "{\"service_id\":" + cabbird::json::Quote(failure.service_id) +
            ",\"error\":" + std::to_string(failure.error) +
            ",\"caused_by\":" + cabbird::json::Quote(failure.caused_by) + '}';
    }
    output += "],\"services\":[";
    for (std::size_t service_index = 0;
        ...
        output += "{\"id\":" + cabbird::json::Quote(service.id) +
            ...
            ",\"lifetime\":" + cabbird::json::Quote(LifetimeName(service.lifetime)) +
            ",\"startup\":" + cabbird::json::Quote(StartupName(service.startup)) +
            ",\"affinity\":" + cabbird::json::Quote(AffinityName(service.affinity)) +
            ",\"state\":" + cabbird::json::Quote(StateName(service.state)) +
        ...
        for (std::size_t dependency_index = 0;
            ...
            output += "{\"id\":" + cabbird::json::Quote(dependency.id) +
            ...
            output += ",\"resolved_version\":" +
                ...
                ",\"state\":" + cabbird::json::Quote(StateName(dependency.state)) + '}';
        }
        ...
        for (std::size_t chain_index = 0;
            ...
            output += cabbird::json::Quote(service.failure_chain[chain_index]);
        }
        output += "]}";
    }
    output += "]}";
