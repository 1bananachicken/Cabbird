...
// ---------------------------------------------------------------------------
// CABBIRD ADDITION -- the only difference from Anomaly's dispatcher.cpp.
//
// The header declares it (include/cabbird/dispatcher.hpp) and it is used by
// src/plugin/scoped_platform_services.cpp.  It exists because a task's terminal state has to be
// nameable in a log line or a diagnostic dump, and a bare enum value is not diagnosable.  It
// reads no state and has no side effects, so it is additive in the strict sense: nothing in the
// ported implementation below depends on it.
//
// This function is all that survives of a previous version of this file that replaced
// Anomaly's implementation wholesale -- its own Entry/PendingCount/Retire internals in place of
// the queued_counts_/in_flight_counts_/RecordTerminalLocked machinery -- and presented Anomaly's
// own three design decisions (the callback runs with the lock released; cancellation is a mark,
// not a removal; a throw is caught and recorded) as its own findings.  The file is upstream's
// again; only this addition is kept.
// ---------------------------------------------------------------------------
...
// CABBIRD ADDITION.  Sums one count table.  Anomaly builds the same total inline inside
// DrainAll's `idle` predicate; it is factored out here for PendingCount rather than copied,
// and DrainAll is left exactly as upstream wrote it.
std::size_t CountAll(const OwnerCounts& values) noexcept {
    std::size_t total{};
...
public:
    ...
    // CABBIRD ADDITION.  Queued-but-not-yet-run tasks, which is what the old rewritten
    // implementation tracked in its own `pending` counter.  Reading upstream's queued_counts_
    // gives the same number without a second source of truth: Post increments it, Pump and the
    // cancellation paths decrement it through Increment/Decrement.
    std::size_t PendingCount() const noexcept {
        std::scoped_lock lock(mutex_);
        return CountAll(queued_counts_);
    }
...
// CABBIRD ADDITION -- see the declaration in cabbird/dispatcher.hpp.
std::size_t Dispatcher::PendingCount() const noexcept {
