...
void ClearInputMailbox(
    EmbeddedInputMailbox& mailbox, cabbird::InputResetReason reason) noexcept {
    mailbox.frame = {};
...
void UpdateModifiers(cabbird::InputFrameState& frame) noexcept {
    ...
    frame.modifiers =
        (shift ? cabbird::ToMask(cabbird::InputModifier::Shift) : 0U) |
        (control ? cabbird::ToMask(cabbird::InputModifier::Control) : 0U) |
        (alt ? cabbird::ToMask(cabbird::InputModifier::Alt) : 0U) |
        (super ? cabbird::ToMask(cabbird::InputModifier::Super) : 0U);
}
...
void RecordKeyboardMessage(
    ...
    if (normalized < cabbird::kInputKeyCount) mailbox.frame.keys[normalized] = down;
...
DrainedInputFrame DrainInputFrame(EmbeddedState& state) noexcept {
    ...
    if (mailbox.reset_pending) {
        const cabbird::InputResetReason reason = mailbox.reset_reason;
        mailbox.reset_pending = false;
        mailbox.reset_reason = cabbird::InputResetReason::None;
        return {{}, reason};
    }

    ReconcileAsyncInput(state, mailbox);
    cabbird::InputFrameState frame = mailbox.frame;
    ...
    mailbox.pending_wheel_delta = 0;
    frame.keys[0] = false;
    return {frame, cabbird::InputResetReason::None};
}

cabbird::InputUiCaptureState CurrentUiCapture() noexcept {
    cabbird::InputUiCaptureState capture;
    if (ImGui::GetCurrentContext() == nullptr || !cabbird::HostUiMenusCaptureMouse()) {
        return capture;
    }
    const ImGuiIO& io = ImGui::GetIO();
    capture.flags = cabbird::ToFlags(cabbird::InputCaptureFlag::Mouse);
    if (io.WantCaptureKeyboard) {
        capture.flags |= cabbird::ToFlags(cabbird::InputCaptureFlag::Keyboard);
    }
    if (io.WantTextInput || io.WantCaptureKeyboard) {
        capture.flags |= cabbird::ToFlags(cabbird::InputCaptureFlag::Text);
    }
...
LRESULT CALLBACK EmbeddedWindowProc(HWND window, UINT message, WPARAM wparam, LPARAM lparam) {
    ...
    if (state != nullptr && state->renderer.load() == RendererLifecycle::Ready &&
        ...
        const bool menus_expanded = !cabbird::HostUiMenusCollapsed();
        const ImGuiIO& io = ImGui::GetIO();
        const bool capture_mouse = cabbird::HostUiMenusCaptureMouse();
...
void RecordEmbeddedInputMessage(
    ...
    try {
        ...
        if (IsFocusLossMessage(message, wparam)) {
            ClearInputMailbox(mailbox, cabbird::InputResetReason::FocusLost);
            return;
        }
...
bool PublishEmbeddedInputFrame(EmbeddedState& state) noexcept {
    ...
    try {
        ...
        if (drained.reset_reason != cabbird::InputResetReason::None) {
        ...
        try {
            ...
            ClearInputMailbox(state.input_mailbox, cabbird::InputResetReason::Explicit);
        } catch (...) {
        }
        return false;
    }
...
void RestoreEmbeddedInput(EmbeddedState& state) noexcept {
    try {
        std::scoped_lock lock(state.input_mailbox.mutex);
        ClearInputMailbox(state.input_mailbox, cabbird::InputResetReason::FocusLost);
    } catch (...) {
    }
    state.input_installed = false;
