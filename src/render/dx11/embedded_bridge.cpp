...
std::unique_ptr<cabbird::HookManager> g_hooks;
...
void EmbeddedPerformanceProbe::Publish(
    const std::shared_ptr<cabbird::StructuredLogger>& logger) noexcept {
    ...
    static constexpr std::array<std::string_view,
        ...
        "platform_ui_unity_compatibility_snapshot", "platform_ui_model_publish",
    ...
    try {
        ...
        details.thread_domain = cabbird::LogThreadDomain::Worker;
        ...
        static_cast<void>(logger->Log(
            cabbird::LogLevel::Info, "runtime-performance",
...
HRESULT STDMETHODCALLTYPE HookPresent(IDXGISwapChain* swap_chain, UINT interval, UINT flags) {
    ...
    const auto render_started = sampled ? std::chrono::steady_clock::now()
                                        : std::chrono::steady_clock::time_point{};
    static cabbird::ThreadLocalScalar<bool> rendering;
...
HRESULT STDMETHODCALLTYPE HookResizeBuffers(
    ...
    const bool affected = static_cast<bool>(callback);
...
bool InstallHooks() {
    ...
    const HookTargets targets = DiscoverD3D11HookTargets();
    if (!targets) return false;
    g_hooks = std::make_unique<cabbird::HookManager>(cabbird::CreateMinHookBackend());
    // The four DXGI targets are the whole set.  They are the surface every Direct3D version
    // shares, so they are required; there is no fifth hook to install.
...
std::string DescribeEmbeddedSubmissionTarget() {
    // There is no submission target to describe on Direct3D11: the immediate context submits
    // through the device, not through a command queue this process could detour.  The function
    // stays because the start-up log asks for it unconditionally, and "none" is the honest
    // answer rather than a second code path that could disagree with InstallHooks().
    return "target=none";
}
...
void RunEmbeddedPlatform(
    ...
    state.diagnostics.performance_probe = [&performance = state.performance](
        ...
        static constexpr std::array<EmbeddedPerformanceStage,
            ...
            EmbeddedPerformanceStage::PlatformUiUnityCompatibilitySnapshot,
    ...
    cabbird::SetHostUiMenusCollapsed(!config.platform_visible);
    ...
    // Borrow the profile-owned adapter. This bridge owns only its tick callback.
    bool tick_bound{};
    bool hooks_owned{};
    bool cleanup_done{};
    ...
    cleanup_guard.callback = [&]() noexcept {
        ...
        try {
            ...
            // ProfileRuntime owns Stop. Drain only this bridge's callback before
            // releasing renderer state; keep services alive for plugin shutdown.
            if (adapter != nullptr && tick_bound) {
            ...
            const bool graphics_active = state.platform_ui_initialized ||
                state.win32_initialized || state.d3d11_device != nullptr;
            if (graphics_active) {
    ...
    if (adapter != nullptr) {
        // The ESC-menu bridge that used to be constructed here is NTE-specific and is not
        // ported; see the note above the performance stage.
        ...
        adapter->SetTickCallback([
            ...
            performance](double delta_seconds) {
            ...
            if (tick_enabled->load(std::memory_order_relaxed)) {
                ...
                // The ESC-menu update is not ported (see above), so this stage is zero
                // rather than missing: the ported enum still has the member.
    ...
    // Missing game bindings degrade game services, not UI startup. Never create or
    // start a second adapter here; the profile lifecycle has already attempted it.
    ...
    state.diagnostics.hooks = [upstream_hooks] {
        auto result = upstream_hooks ? upstream_hooks() : std::vector<cabbird::HookRecordView>{};
        if (g_hooks != nullptr) {
    ...
    std::ofstream(root / L"cabbird-platform.log", std::ios::app)
        // Named for what it actually installs.  This read `d3d12_hooks=` since the D3D12 days and
        // was never renamed when the renderer moved to D3D11, so anyone diagnosing a D3D11 title
        // saw "d3d12_hooks=1" and had to go read InstallHooks() to discover that was misleading:
        // what gets installed is IDXGISwapChain::Present / Present1 from
        // DiscoverD3D11HookTargets(), and no D3D12 interface appears anywhere in this file.
        << "pid=" << GetCurrentProcessId() << " dxgi_present_hooks=" << (installed ? 1 : 0)
        << " plugins_decoupled=1\n";
    if (!installed) {
        return;
    }
