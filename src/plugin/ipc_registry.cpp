...
CabbirdStatusV1 Status(
    ...
    const CabbirdIpcErrorV1 detail = CABBIRD_IPC_ERROR_V1_NONE,
...
bool HashEqual(
    const CabbirdIpcSchemaHashV1& left, const CabbirdIpcSchemaHashV1& right) noexcept {
    return std::memcmp(left.bytes, right.bytes, CABBIRD_IPC_SCHEMA_HASH_V1_SIZE) == 0;
}

bool HashPresent(const CabbirdIpcSchemaHashV1& hash) noexcept {
    return std::ranges::any_of(hash.bytes, [](const std::uint8_t value) { return value != 0; });
}

std::string HashHex(const CabbirdIpcSchemaHashV1& hash) {
    constexpr char digits[] = "0123456789abcdef";
    std::string result(CABBIRD_IPC_SCHEMA_HASH_V1_SIZE * 2U, '0');
    for (std::size_t index = 0; index < CABBIRD_IPC_SCHEMA_HASH_V1_SIZE; ++index) {
...
bool ValidSpan(const CabbirdByteSpanV1 span, const std::size_t maximum) noexcept {
    return (span.data != nullptr || span.size == 0) && span.size <= maximum;
}

std::vector<std::uint8_t> CopySpan(const CabbirdByteSpanV1 span) {
    return span.size == 0
...
bool Owns(
    ...
    const CabbirdGenerationHandleV1 handle,
...
IpcCallingDomain DomainForAffinity(const std::uint32_t affinity) noexcept {
    switch (affinity) {
    case CABBIRD_IPC_AFFINITY_V1_LIFECYCLE: return IpcCallingDomain::Lifecycle;
    case CABBIRD_IPC_AFFINITY_V1_WORKER: return IpcCallingDomain::Worker;
    case CABBIRD_IPC_AFFINITY_V1_GAME: return IpcCallingDomain::Game;
    case CABBIRD_IPC_AFFINITY_V1_RENDER: return IpcCallingDomain::Render;
...
public:
    struct Endpoint final {
        ...
        CabbirdIpcRequestHandlerV1 handler{};
        void* callback_user{};
    ...
    bool Post(
        ...
        try {
            if (affinity != CABBIRD_IPC_AFFINITY_V1_CALLER &&
                affinity != CABBIRD_IPC_AFFINITY_V1_WORKER && external_post_) {
    ...
    std::shared_ptr<Endpoint> Resolve(
        ...
        if (!endpoint || !endpoint->active.load(std::memory_order_acquire)) {
            status = Status(CABBIRD_STATUS_V1_NOT_FOUND,
                CABBIRD_IPC_ERROR_V1_PROVIDER_MISSING, "IPC provider is missing");
            return {};
        }
        if (!HasDependency(consumer, endpoint->owner)) {
            status = Status(CABBIRD_STATUS_V1_PERMISSION_DENIED,
                CABBIRD_IPC_ERROR_V1_DEPENDENCY_REQUIRED,
        ...
        if (selector->major_version != endpoint->major ||
            ...
            status = Status(CABBIRD_STATUS_V1_CONFLICT,
                CABBIRD_IPC_ERROR_V1_VERSION_MISMATCH, "IPC endpoint version mismatch");
            return {};
        }
        const bool schema_matches = mode == CABBIRD_IPC_MODE_V1_EVENT
        ...
        if (!schema_matches) {
            status = Status(CABBIRD_STATUS_V1_CONFLICT,
                CABBIRD_IPC_ERROR_V1_SCHEMA_MISMATCH, "IPC endpoint schema mismatch");
            return {};
        }
        if ((endpoint->modes & mode) == 0) {
            status = Status(CABBIRD_STATUS_V1_UNAVAILABLE,
                CABBIRD_IPC_ERROR_V1_MODE_UNAVAILABLE, "IPC call mode is unavailable");
            return {};
        }
        {
            std::scoped_lock lock(mutex_);
        ...
        status = Status(CABBIRD_STATUS_V1_OK);
        return endpoint;
    }

    CabbirdStatusV1 RunRequest(
        const IpcPluginOwner& caller,
        const std::shared_ptr<Endpoint>& endpoint,
        const std::uint64_t request_id,
        const CabbirdByteSpanV1 request,
        const CabbirdMutableByteSpanV1 response,
        std::size_t* response_size) noexcept {
        if (!endpoint || !endpoint->handler) {
            return Status(CABBIRD_STATUS_V1_UNAVAILABLE,
                CABBIRD_IPC_ERROR_V1_MODE_UNAVAILABLE, "IPC request handler is unavailable");
        }
        ...
        if (!lease || !endpoint->active.load(std::memory_order_acquire)) {
            return Status(CABBIRD_STATUS_V1_CANCELLED,
                CABBIRD_IPC_ERROR_V1_STALE_GENERATION, "IPC provider generation is stale");
        }
        const std::string& caller_id = caller.scope->Owner();
        const CabbirdIpcRequestContextV1 context{
            sizeof(CabbirdIpcRequestContextV1), 0, request_id,
            {caller_id.data(), caller_id.size()}};
        const auto started = std::chrono::steady_clock::now();
        CabbirdStatusV1 result = Status(CABBIRD_STATUS_V1_FAILED);
        try {
            result = endpoint->handler(
                endpoint->callback_user, &context, request, response, response_size);
        } catch (...) {
            result = Status(CABBIRD_STATUS_V1_FAILED, CABBIRD_IPC_ERROR_V1_NONE,
            ...
            std::chrono::steady_clock::now() - started).count();
        {
            std::scoped_lock lock(mutex_);
            ++endpoint->calls;
            if (result.code != CABBIRD_STATUS_V1_OK) ++endpoint->failures;
                ...
                if (lease && pending->active.load(std::memory_order_acquire)) {
                    try {
                        pending->completion(
                            ...
                            Status(CABBIRD_STATUS_V1_TIMEOUT, CABBIRD_IPC_ERROR_V1_TIMEOUT,
...
CabbirdStatusV1 IpcRegistry::RegisterEndpoint(
    const IpcPluginOwner& owner,
    const CabbirdIpcEndpointDescriptorV1* descriptor,
    const CabbirdIpcRequestHandlerV1 request_handler,
    void* callback_user,
    ...
    const std::uint32_t request_modes = CABBIRD_IPC_MODE_V1_SYNC_REQUEST |
        CABBIRD_IPC_MODE_V1_ASYNC_REQUEST;
    const bool valid_modes = descriptor->modes != 0 &&
        (descriptor->modes & ~(request_modes | CABBIRD_IPC_MODE_V1_EVENT)) == 0;
    const bool valid_affinity = descriptor->affinity <= CABBIRD_IPC_AFFINITY_V1_RENDER;
    const bool valid_reentrancy = descriptor->reentrancy <= CABBIRD_IPC_REENTRANCY_V1_ALLOW;
    if (!IsReverseDomainId(id) || descriptor->major_version == 0 || !valid_modes ||
        ...
        ((descriptor->modes & CABBIRD_IPC_MODE_V1_EVENT) != 0 &&
    ...
    try {
        ...
        state->callback_user = callback_user;
        {
            ...
            if (impl_->endpoints_.contains(state->id)) {
                return Status(CABBIRD_STATUS_V1_CONFLICT,
                    CABBIRD_IPC_ERROR_V1_VERSION_MISMATCH,
        ...
        if (state->token == 0) {
            return Status(CABBIRD_STATUS_V1_UNAVAILABLE,
                CABBIRD_IPC_ERROR_V1_STALE_GENERATION, "plugin scope is stopping");
        }
        {
            std::scoped_lock lock(impl_->mutex_);
...
CabbirdStatusV1 IpcRegistry::UnregisterEndpoint(
    const IpcPluginOwner& owner, const CabbirdGenerationHandleV1 endpoint) noexcept {
    ...
    if (!state || !Owns(owner, state->owner.scope, endpoint, state->token)) {
        return Status(CABBIRD_STATUS_V1_NOT_FOUND,
            CABBIRD_IPC_ERROR_V1_STALE_GENERATION, "IPC endpoint handle is stale");
    }
    return owner.scope->Release(endpoint.id)
        ? Status(CABBIRD_STATUS_V1_OK)
        : Status(CABBIRD_STATUS_V1_NOT_FOUND, CABBIRD_IPC_ERROR_V1_STALE_GENERATION);
}

CabbirdStatusV1 IpcRegistry::Invoke(
    const IpcPluginOwner& owner,
    ...
    const CabbirdMutableByteSpanV1 response,
    std::size_t* response_size) noexcept {
    CabbirdStatusV1 resolved{};
    const auto endpoint = impl_->Resolve(owner, selector, CABBIRD_IPC_MODE_V1_SYNC_REQUEST, resolved);
    ...
    if (calling_domain == IpcCallingDomain::Game || calling_domain == IpcCallingDomain::Render) {
        return Status(CABBIRD_STATUS_V1_UNAVAILABLE,
            CABBIRD_IPC_ERROR_V1_MODE_UNAVAILABLE,
    ...
    if (endpoint->reentrancy == CABBIRD_IPC_REENTRANCY_V1_REJECT &&
        !impl_->AddWaitEdge(caller, provider)) {
        return Status(CABBIRD_STATUS_V1_CONFLICT,
            CABBIRD_IPC_ERROR_V1_REENTRANT_CYCLE, "IPC reentrant cycle rejected");
    }
    ...
    } edge{endpoint->reentrancy == CABBIRD_IPC_REENTRANCY_V1_REJECT ? impl_.get() : nullptr,
           caller, provider};
    const std::uint64_t request_id = impl_->next_request_id_.fetch_add(1);
    if (endpoint->affinity == CABBIRD_IPC_AFFINITY_V1_CALLER ||
    ...
    auto promise = std::make_shared<std::promise<CabbirdStatusV1>>();
    std::future<CabbirdStatusV1> future = promise->get_future();
    const auto reservation = impl_->ReserveQueue(endpoint);
    if (!reservation) {
        return Status(CABBIRD_STATUS_V1_UNAVAILABLE,
            CABBIRD_IPC_ERROR_V1_QUEUE_FULL, "IPC endpoint queue is full");
    }
    ...
    if (!posted) {
        return Status(CABBIRD_STATUS_V1_UNAVAILABLE,
            CABBIRD_IPC_ERROR_V1_MODE_UNAVAILABLE, "IPC affinity dispatcher is unavailable");
    }
    if (future.wait_for(std::chrono::milliseconds(endpoint->timeout_milliseconds)) !=
        ...
        return Status(CABBIRD_STATUS_V1_TIMEOUT,
            CABBIRD_IPC_ERROR_V1_TIMEOUT, "IPC request timed out");
    }
    const CabbirdStatusV1 result = future.get();
...
CabbirdStatusV1 IpcRegistry::InvokeAsync(
    const IpcPluginOwner& owner,
    const CabbirdIpcEndpointSelectorV1* selector,
    const CabbirdByteSpanV1 request,
    const CabbirdIpcCompletionCallbackV1 completion,
    void* completion_user,
    CabbirdGenerationHandleV1* pending_call) noexcept {
    CabbirdStatusV1 resolved{};
    const auto endpoint = impl_->Resolve(owner, selector, CABBIRD_IPC_MODE_V1_ASYNC_REQUEST, resolved);
    if (!endpoint) return resolved;
    ...
    try {
        ...
        const bool posted = impl_->Post(
            ...
            [impl = impl_, pending, bytes, reservation] {
                ...
                CabbirdStatusV1 status = Status(CABBIRD_STATUS_V1_FAILED);
                try {
                    ...
                    status = Status(
                        CABBIRD_STATUS_V1_FAILED, CABBIRD_IPC_ERROR_V1_NONE,
...
CabbirdStatusV1 IpcRegistry::Cancel(
    const IpcPluginOwner& owner, const CabbirdGenerationHandleV1 pending_call) noexcept {
    std::shared_ptr<Impl::Pending> pending;
    {
        std::scoped_lock lock(impl_->mutex_);
    ...
    if (!pending || !Owns(owner, pending->owner.scope, pending_call, pending->token)) {
        return Status(CABBIRD_STATUS_V1_NOT_FOUND,
            CABBIRD_IPC_ERROR_V1_STALE_GENERATION, "IPC pending handle is stale");
    }
...
CabbirdStatusV1 IpcRegistry::Subscribe(
    const IpcPluginOwner& owner,
    const CabbirdIpcEndpointSelectorV1* selector,
    const CabbirdIpcEventCallbackV1 callback,
    void* callback_user,
    CabbirdGenerationHandleV1* subscription) noexcept {
    CabbirdStatusV1 resolved{};
    const auto endpoint = impl_->Resolve(owner, selector, CABBIRD_IPC_MODE_V1_EVENT, resolved);
    if (!endpoint) return resolved;
...
CabbirdStatusV1 IpcRegistry::Unsubscribe(
    const IpcPluginOwner& owner, const CabbirdGenerationHandleV1 subscription) noexcept {
    ...
    if (!state || !Owns(owner, state->owner.scope, subscription, state->token)) {
        return Status(CABBIRD_STATUS_V1_NOT_FOUND,
            CABBIRD_IPC_ERROR_V1_STALE_GENERATION, "IPC subscription handle is stale");
    }
...
CabbirdStatusV1 IpcRegistry::Publish(
    const IpcPluginOwner& owner,
    const CabbirdGenerationHandleV1 endpoint_handle,
    const CabbirdByteSpanV1 event) noexcept {
    ...
    if ((endpoint->modes & CABBIRD_IPC_MODE_V1_EVENT) == 0 ||
