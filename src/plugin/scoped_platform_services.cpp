...
#include "cabbird/reliable_storage.hpp"
...
CabbirdStatusV1 Status(const std::uint32_t code, const char* message = nullptr) noexcept {
    return {code, 0, {message, message == nullptr ? 0U : std::strlen(message)}};
}

CabbirdStatusV1 InvalidArgument(const char* message = "invalid argument") noexcept {
    return Status(CABBIRD_STATUS_V1_INVALID_ARGUMENT, message);
}
...
std::span<const std::byte> AsBytes(const CabbirdByteSpanV1 value) noexcept {
    return std::as_bytes(std::span(value.data, value.size));
}

CabbirdStatusV1 CopyBytes(
    const std::span<const std::uint8_t> source,
    const CabbirdMutableByteSpanV1 destination,
    ...
    if (*inout_size < required || destination.size < required) {
        *inout_size = required;
        return Status(CABBIRD_STATUS_V1_BUFFER_TOO_SMALL, "destination is too small");
    }
...
CabbirdStatusV1 StorageStatus(const StorageResult& result) noexcept {
    if (result.ok()) return Status(CABBIRD_STATUS_V1_OK);
    if (result.win32_error == ERROR_FILE_NOT_FOUND || result.win32_error == ERROR_PATH_NOT_FOUND) {
        return Status(CABBIRD_STATUS_V1_NOT_FOUND, "storage entry was not found");
    }
    ...
    if (result.error == StorageError::SizeLimitExceeded) {
        return Status(CABBIRD_STATUS_V1_FAILED, "storage entry exceeds the service limit");
    }
    return Status(CABBIRD_STATUS_V1_FAILED, "storage operation failed");
}
...
bool Owns(const std::shared_ptr<State>& state, const ScopedPluginServiceOwner& owner,
          const CabbirdGenerationHandleV1 handle) noexcept {
...
CabbirdStatusV1 EnsureStateDirectory(const ScopedPluginServiceOwner& owner) noexcept {
    ...
    try {
        ...
        if (error || !std::filesystem::is_directory(owner.state_directory, error) || error) {
            return Status(CABBIRD_STATUS_V1_FAILED, "plugin state root is unavailable");
        }
        return Status(CABBIRD_STATUS_V1_OK);
    } catch (...) {
        return Status(CABBIRD_STATUS_V1_FAILED, "plugin state root is unavailable");
...
CabbirdStatusV1 EnsureConfigurationDirectory(const ScopedPluginServiceOwner& owner) noexcept {
    ...
    try {
        ...
        if (error || !std::filesystem::is_directory(owner.configuration_directory, error) || error) {
            return Status(CABBIRD_STATUS_V1_FAILED, "plugin configuration root is unavailable");
        }
        return Status(CABBIRD_STATUS_V1_OK);
    } catch (...) {
        return Status(CABBIRD_STATUS_V1_FAILED, "plugin configuration root is unavailable");
...
CabbirdStatusV1 ValidateConfigDocument(
    const ScopedPlatformServices::Impl::ConfigState& config,
    const CabbirdByteSpanV1 document) noexcept {
    ...
    try {
        if (!config.validator) return Status(CABBIRD_STATUS_V1_FAILED, "configuration schema is unavailable");
        const Json value = Json::parse(document.data, document.data + document.size);
        config.validator->validate(value);
        return Status(CABBIRD_STATUS_V1_OK);
    } catch (...) {
        return Status(CABBIRD_STATUS_V1_FAILED, "configuration does not satisfy its schema");
    }
}

CabbirdStatusV1 ReadConfigEnvelope(
    const ScopedPluginServiceOwner& owner,
    const ScopedPlatformServices::Impl::ConfigState& config,
    std::uint32_t* version,
    std::vector<std::uint8_t>& document) noexcept {
    if (version == nullptr) return InvalidArgument();
    const CabbirdStatusV1 configuration_root = EnsureConfigurationDirectory(owner);
    if (configuration_root.code != CABBIRD_STATUS_V1_OK) return configuration_root;
    ReliableStorage storage(owner.configuration_directory.wstring());
    if (!storage.InitializationResult()) {
        return Status(CABBIRD_STATUS_V1_FAILED, "configuration storage unavailable");
    }
    const StorageReadResult read = storage.Read(ConfigPath(config.id), kMaximumConfigEnvelopeBytes);
    const CabbirdStatusV1 storage_status = StorageStatus(read.result);
    if (storage_status.code != CABBIRD_STATUS_V1_OK) return storage_status;
    try {
        ...
        if (!parsed.is_object() || !parsed.contains("schemaVersion") || !parsed.contains("data") ||
            ...
            return Status(CABBIRD_STATUS_V1_FAILED, "stored configuration envelope is invalid");
        }
        const std::uint64_t stored_version = parsed["schemaVersion"].get<std::uint64_t>();
        if (stored_version == 0 || stored_version > UINT32_MAX) {
            return Status(CABBIRD_STATUS_V1_FAILED, "stored configuration version is invalid");
        }
        const std::string data = parsed["data"].dump();
        if (data.size() > kMaximumConfigBytes) {
            return Status(CABBIRD_STATUS_V1_FAILED, "stored configuration is too large");
        }
        *version = static_cast<std::uint32_t>(stored_version);
        document.assign(data.begin(), data.end());
        return Status(CABBIRD_STATUS_V1_OK);
    } catch (...) {
        return Status(CABBIRD_STATUS_V1_FAILED, "stored configuration is not valid JSON");
    }
}
...
CabbirdStatusV1 ScopedPlatformServices::RegisterConfigSchema(
    const ScopedPluginServiceOwner& owner,
    const std::string_view schema_id,
    const std::uint32_t schema_version,
    const CabbirdByteSpanV1 schema_json,
    ...
    try {
        ...
        bool inserted{};
        {
            ...
            if (duplicate) {
                return Status(CABBIRD_STATUS_V1_CONFLICT, "configuration schema is already registered");
            }
            state->token = owner.scope->Register(
                PluginResourceKind::Config, "config.schema:" + state->id,
                [this, state] { RemoveConfig(*impl_, state); });
            if (state->token == 0) {
                return Status(CABBIRD_STATUS_V1_UNAVAILABLE, "plugin scope is stopping");
            }
            inserted = impl_->configs_.emplace(state->token, state).second;
        }
        if (!inserted) {
            static_cast<void>(owner.scope->Release(state->token));
            return Status(CABBIRD_STATUS_V1_FAILED, "configuration schema registration failed");
        }
        *handle = {state->token, owner.scope->Generation()};
        return Status(CABBIRD_STATUS_V1_OK);
    } catch (...) {
        if (state && state->token != 0) static_cast<void>(owner.scope->Release(state->token));
        return Status(CABBIRD_STATUS_V1_FAILED, "configuration schema is not valid");
    }
}

CabbirdStatusV1 ScopedPlatformServices::UnregisterConfigSchema(
    const ScopedPluginServiceOwner& owner, const CabbirdGenerationHandleV1 handle) noexcept {
    std::shared_ptr<Impl::ConfigState> state;
    {
        std::scoped_lock lock(impl_->mutex_);
    ...
    if (!Owns(state, owner, handle)) return Status(CABBIRD_STATUS_V1_NOT_FOUND, "configuration schema handle is stale");
    return owner.scope->Release(handle.id)
        ? Status(CABBIRD_STATUS_V1_OK)
        : Status(CABBIRD_STATUS_V1_NOT_FOUND, "configuration schema handle is stale");
}

CabbirdStatusV1 ScopedPlatformServices::ReadConfig(
    const ScopedPluginServiceOwner& owner,
    const std::string_view schema_id,
    ...
    if (!config) return Status(CABBIRD_STATUS_V1_NOT_FOUND, "configuration schema is not registered");
    std::vector<std::uint8_t> document;
    const CabbirdStatusV1 status = ReadConfigEnvelope(owner, *config, schema_version, document);
    if (status.code != CABBIRD_STATUS_V1_OK) return status;
    return CopyBytes(document, destination, inout_size);
}

CabbirdStatusV1 ScopedPlatformServices::WriteConfig(
    const ScopedPluginServiceOwner& owner,
    const std::string_view schema_id,
    const std::uint32_t schema_version,
    const CabbirdByteSpanV1 document) noexcept {
    ...
    if (config->version != schema_version) {
        return Status(CABBIRD_STATUS_V1_CONFLICT, "configuration schema version is not registered");
    }
    const CabbirdStatusV1 validation = ValidateConfigDocument(*config, document);
    if (validation.code != CABBIRD_STATUS_V1_OK) return validation;
    try {
        const Json value = Json::parse(document.data, document.data + document.size);
        if (value.dump().size() > kMaximumConfigBytes) {
            return Status(CABBIRD_STATUS_V1_FAILED, "canonical configuration is too large");
        }
        Json envelope;
        ...
        if (serialized.size() > kMaximumConfigEnvelopeBytes) {
            return Status(CABBIRD_STATUS_V1_FAILED, "configuration envelope is too large");
        }
        ReliableStorage storage(owner.configuration_directory.wstring());
        if (!storage.InitializationResult()) {
            return Status(CABBIRD_STATUS_V1_FAILED, "configuration storage unavailable");
        ...
        return Status(CABBIRD_STATUS_V1_FAILED, "configuration write failed");
    }
}

CabbirdStatusV1 ScopedPlatformServices::MigrateConfig(
    const ScopedPluginServiceOwner& owner,
    const std::string_view schema_id,
    const CabbirdConfigMigrationV1 migration,
    ...
    const CabbirdStatusV1 read = ReadConfigEnvelope(owner, *config, &stored_version, source);
    if (read.code != CABBIRD_STATUS_V1_OK) return read;
    if (stored_version == config->version) return Status(CABBIRD_STATUS_V1_OK);
    std::size_t required{};
    CabbirdStatusV1 result{};
    try {
        result = migration(migration_user, stored_version, {source.data(), source.size()}, {nullptr, 0}, &required);
    } catch (...) {
        return Status(CABBIRD_STATUS_V1_FAILED, "configuration migration raised an exception");
    }
    if ((result.code != CABBIRD_STATUS_V1_OK && result.code != CABBIRD_STATUS_V1_BUFFER_TOO_SMALL) ||
        required == 0 || required > kMaximumCallbackOutputBytes) {
        return Status(CABBIRD_STATUS_V1_FAILED, "configuration migration did not provide an output size");
    }
    ...
    if (result.code != CABBIRD_STATUS_V1_OK || required > destination.size()) {
        return Status(CABBIRD_STATUS_V1_FAILED, "configuration migration failed");
    }
    destination.resize(required);
    return WriteConfig(owner, schema_id, config->version, {destination.data(), destination.size()});
}

CabbirdStatusV1 ScopedPlatformServices::ReadStorage(
    const ScopedPluginServiceOwner& owner,
    ...
    try {
        const CabbirdStatusV1 state_root = EnsureStateDirectory(owner);
        if (state_root.code != CABBIRD_STATUS_V1_OK) return state_root;
        ReliableStorage storage(owner.state_directory.wstring());
        if (!storage.InitializationResult()) return Status(CABBIRD_STATUS_V1_FAILED, "state storage unavailable");
        const StorageReadResult read = storage.Read(StoragePath(relative_path), kMaximumStorageBytes);
        const CabbirdStatusV1 status = StorageStatus(read.result);
        if (status.code != CABBIRD_STATUS_V1_OK) return status;
        return CopyBytes(
            std::span(reinterpret_cast<const std::uint8_t*>(read.bytes.data()), read.bytes.size()),
            destination, inout_size);
    } catch (...) {
        return Status(CABBIRD_STATUS_V1_FAILED, "storage read failed");
    }
}

CabbirdStatusV1 ScopedPlatformServices::WriteStorage(
    const ScopedPluginServiceOwner& owner,
    const std::string_view relative_path,
    const CabbirdByteSpanV1 source) noexcept {
    ...
    try {
        ...
        return Status(CABBIRD_STATUS_V1_FAILED, "storage write failed");
    }
}

CabbirdStatusV1 ScopedPlatformServices::RemoveStorage(
    ...
    try {
        ...
        return Status(CABBIRD_STATUS_V1_FAILED, "storage delete failed");
    }
}

CabbirdStatusV1 ScopedPlatformServices::RuntimeInfo(
    ...
    snapshot->runtime_version_major = CABBIRD_SDK_VERSION_MAJOR;
    snapshot->runtime_version_minor = CABBIRD_SDK_VERSION_MINOR;
    snapshot->runtime_version_patch = CABBIRD_SDK_VERSION_PATCH;
...
CabbirdStatusV1 ScopedPlatformServices::RuntimeVersion(
    char* destination, std::size_t* inout_size) noexcept {
    return CopyString(CABBIRD_SDK_VERSION_STRING, destination, inout_size);
}

CabbirdStatusV1 ScopedPlatformServices::RegisterSelfTest(
    const ScopedPluginServiceOwner& owner,
    const std::string_view id,
    const CabbirdDiagnosticSelfTestV1 callback,
    void* callback_user,
    CabbirdGenerationHandleV1* handle) noexcept {
    ...
    try {
        ...
        if (state->token == 0) return Status(CABBIRD_STATUS_V1_UNAVAILABLE, "plugin scope is stopping");
        bool duplicate{};
        {
            std::scoped_lock lock(impl_->mutex_);
        ...
        if (duplicate) {
            static_cast<void>(owner.scope->Release(state->token));
            return Status(CABBIRD_STATUS_V1_CONFLICT, "self-test id is already registered");
        }
        *handle = {state->token, owner.scope->Generation()};
        return Status(CABBIRD_STATUS_V1_OK);
    } catch (...) {
        return Status(CABBIRD_STATUS_V1_FAILED, "self-test registration failed");
    }
}

CabbirdStatusV1 ScopedPlatformServices::UnregisterSelfTest(
    const ScopedPluginServiceOwner& owner, const CabbirdGenerationHandleV1 handle) noexcept {
    ...
    if (!Owns(state, owner, handle)) return Status(CABBIRD_STATUS_V1_NOT_FOUND, "self-test handle is stale");
    return owner.scope->Release(handle.id)
        ? Status(CABBIRD_STATUS_V1_OK)
        : Status(CABBIRD_STATUS_V1_NOT_FOUND, "self-test handle is stale");
}

CabbirdStatusV1 ScopedPlatformServices::RunSelfTest(
    const ScopedPluginServiceOwner& owner,
    const std::string_view id,
    const CabbirdMutableByteSpanV1 destination,
    std::size_t* inout_size) noexcept {
    ...
    if (!state) return Status(CABBIRD_STATUS_V1_NOT_FOUND, "self-test is not registered");
    auto lease = state->owner.scope->AcquireCallback(state->owner.scope->Generation());
    if (!lease) return Status(CABBIRD_STATUS_V1_UNAVAILABLE, "plugin scope is stopping");
    try {
        return state->callback(state->callback_user, destination, inout_size);
    } catch (...) {
        return Status(CABBIRD_STATUS_V1_FAILED, "self-test raised an exception");
    }
}

CabbirdStatusV1 ScopedPlatformServices::DiagnosticsSnapshot(
    const ScopedPluginServiceOwner& owner,
    const CabbirdMutableByteSpanV1 destination,
    std::size_t* inout_size) noexcept {
    ...
    try {
        ...
        return Status(CABBIRD_STATUS_V1_FAILED, "diagnostics snapshot failed");
    }
}
...
CabbirdStatusV1 ScopedPlatformServices::Schedule(
    const ScopedPluginServiceOwner& owner,
    const std::uint32_t delay_milliseconds,
    const CabbirdTaskCallbackV1 callback,
    void* callback_user,
    CabbirdGenerationHandleV1* handle) noexcept {
    ...
    try {
        ...
        const CabbirdGenerationHandleV1 task{state->token, owner.scope->Generation()};
        impl_->Enqueue(
        ...
        *handle = task;
        ...
        return Status(CABBIRD_STATUS_V1_FAILED, "task scheduling failed");
    }
}

CabbirdStatusV1 ScopedPlatformServices::CancelTask(
    const ScopedPluginServiceOwner& owner, const CabbirdGenerationHandleV1 handle) noexcept {
    ...
    if (!Owns(state, owner, handle)) return Status(CABBIRD_STATUS_V1_NOT_FOUND, "task handle is stale");
    return owner.scope->Release(handle.id)
        ? Status(CABBIRD_STATUS_V1_CANCELLED)
        : Status(CABBIRD_STATUS_V1_NOT_FOUND, "task handle is stale");
}

CabbirdStatusV1 ScopedPlatformServices::RegisterCommand(
    const ScopedPluginServiceOwner& owner,
    const std::string_view name,
    const std::string_view description,
    const CabbirdCommandCallbackV1 callback,
    void* callback_user,
    CabbirdGenerationHandleV1* handle) noexcept {
    ...
    try {
        {
            ...
            if (duplicate) return Status(CABBIRD_STATUS_V1_CONFLICT, "command name is already registered");
        }
        ...
        *handle = {state->token, owner.scope->Generation()};
        ...
        return Status(CABBIRD_STATUS_V1_FAILED, "command registration failed");
    }
}

CabbirdStatusV1 ScopedPlatformServices::UnregisterCommand(
    const ScopedPluginServiceOwner& owner, const CabbirdGenerationHandleV1 handle) noexcept {
    ...
    if (!Owns(state, owner, handle)) return Status(CABBIRD_STATUS_V1_NOT_FOUND, "command handle is stale");
    return owner.scope->Release(handle.id)
        ? Status(CABBIRD_STATUS_V1_OK)
        : Status(CABBIRD_STATUS_V1_NOT_FOUND, "command handle is stale");
}

CabbirdStatusV1 ScopedPlatformServices::InvokeCommand(
    const ScopedPluginServiceOwner& owner,
    const std::string_view name,
    ...
    if (!state) return Status(CABBIRD_STATUS_V1_NOT_FOUND, "command is not registered");
    auto lease = state->owner.scope->AcquireCallback(state->owner.scope->Generation());
    if (!lease) return Status(CABBIRD_STATUS_V1_UNAVAILABLE, "command owner is stopping");
    const auto started = std::chrono::steady_clock::now();
    bool fault{};
    CabbirdStatusV1 result{};
    try {
        ...
        fault = true;
        result = Status(CABBIRD_STATUS_V1_FAILED, "command callback raised an exception");
    }
    impl_->RecordCallback(state->owner, fault, std::chrono::steady_clock::now() - started);
    return result;
}

CabbirdStatusV1 ScopedPlatformServices::PostNotification(
    const ScopedPluginServiceOwner& owner,
    const CabbirdNotificationSeverityV1 severity,
    ...
    if (owner.scope == nullptr || handle == nullptr || !IsText(title) || body.find('\0') != std::string_view::npos ||
        title.size() > 256 || body.size() > 4096 || severity > CABBIRD_NOTIFICATION_V1_ERROR) {
        return InvalidArgument();
    }
    try {
        ...
        *handle = {state->token, owner.scope->Generation()};
        ...
        return Status(CABBIRD_STATUS_V1_FAILED, "notification post failed");
    }
}

CabbirdStatusV1 ScopedPlatformServices::DismissNotification(
    const ScopedPluginServiceOwner& owner, const CabbirdGenerationHandleV1 handle) noexcept {
    ...
    if (!Owns(state, owner, handle)) return Status(CABBIRD_STATUS_V1_NOT_FOUND, "notification handle is stale");
    return owner.scope->Release(handle.id)
        ? Status(CABBIRD_STATUS_V1_OK)
        : Status(CABBIRD_STATUS_V1_NOT_FOUND, "notification handle is stale");
}

CabbirdStatusV1 ScopedPlatformServices::ResolveSignature(
    const ScopedPluginServiceOwner& owner,
    ...
    try {
        ...
        if (!module_info) return Status(CABBIRD_STATUS_V1_NOT_FOUND, "module is not loaded");
        const unitymem::Pattern parsed = impl_->memory_services_.patterns->Parse(pattern);
        const auto matches = impl_->memory_services_.patterns->ScanSection(
            *module_info, section_name, parsed, 2);
        if (matches.empty()) return Status(CABBIRD_STATUS_V1_NOT_FOUND, "signature did not match");
        if (matches.size() != 1) return Status(CABBIRD_STATUS_V1_CONFLICT, "signature matched multiple targets");
        *address = matches.front();
        return Status(CABBIRD_STATUS_V1_OK);
    } catch (...) {
        return Status(CABBIRD_STATUS_V1_FAILED, "signature preflight failed");
    }
}

CabbirdStatusV1 ScopedPlatformServices::CreateHook(
    const ScopedPluginServiceOwner& owner,
    const CabbirdHookRequestV1* request,
    std::uintptr_t* original,
    CabbirdGenerationHandleV1* handle) noexcept {
    constexpr std::size_t request_size = offsetof(CabbirdHookRequestV1, label) +
        sizeof(CabbirdHookRequestV1::label);
    ...
    if (request->kind != CABBIRD_HOOK_V1_FUNCTION) {
        return Status(CABBIRD_STATUS_V1_UNAVAILABLE, "hook kind requires a dedicated adapter");
    }
    if (!IsExecutableAddress(request->target)) {
        return Status(CABBIRD_STATUS_V1_FAILED, "hook target failed executable preflight");
    }
    ...
    try {
        ...
        if (!created) {
            ...
            return removed
                ? Status(CABBIRD_STATUS_V1_CONFLICT, "hook target conflicts with an existing hook")
                : Status(CABBIRD_STATUS_V1_FAILED, "hook setup cleanup failed");
        }
        return Status(CABBIRD_STATUS_V1_OK);
    } catch (...) {
        ...
        return Status(CABBIRD_STATUS_V1_FAILED, "hook creation failed");
    }
}

CabbirdStatusV1 ScopedPlatformServices::ReleaseHook(
    const ScopedPluginServiceOwner& owner, const CabbirdGenerationHandleV1 handle) noexcept {
    std::shared_ptr<Impl::HookState> state;
    {
        std::scoped_lock lock(impl_->mutex_);
    ...
    if (!Owns(state, owner, handle)) return Status(CABBIRD_STATUS_V1_NOT_FOUND, "hook handle is stale");
    if (!RemoveHook(*impl_, state, DefaultHookRemovalDeadline())) {
        return Status(CABBIRD_STATUS_V1_TIMEOUT, "hook callback did not drain");
    }
    return owner.scope->Release(handle.id)
        ? Status(CABBIRD_STATUS_V1_OK)
        : Status(CABBIRD_STATUS_V1_NOT_FOUND, "hook handle is stale");
}

CabbirdStatusV1 ScopedPlatformServices::BeginHookCallback(
    const ScopedPluginServiceOwner& owner,
    const CabbirdGenerationHandleV1 hook,
    ...
    if (!Owns(state, owner, hook)) return Status(CABBIRD_STATUS_V1_NOT_FOUND, "hook handle is stale");
    if (!impl_->hooks_) return Status(CABBIRD_STATUS_V1_UNAVAILABLE, "hook service is unavailable");
    auto hook_lease = impl_->hooks_->AcquireCallback(
        owner.scope->Owner(), owner.scope->Generation(), reinterpret_cast<void*>(state->target));
    if (!hook_lease) return Status(CABBIRD_STATUS_V1_UNAVAILABLE, "hook callback source is stopping");
    auto plugin_lease = owner.scope->AcquireCallback(owner.scope->Generation());
    if (!plugin_lease) return Status(CABBIRD_STATUS_V1_UNAVAILABLE, "plugin scope is stopping");
    try {
        std::scoped_lock lock(impl_->mutex_);
        ...
        *callback_lease = {id, owner.scope->Generation()};
        ...
        return Status(CABBIRD_STATUS_V1_FAILED, "hook callback lease registration failed");
    }
}

CabbirdStatusV1 ScopedPlatformServices::EndHookCallback(
    const ScopedPluginServiceOwner& owner,
    const CabbirdGenerationHandleV1 callback_lease) noexcept {
    if (owner.scope == nullptr) return InvalidArgument();
    std::scoped_lock lock(impl_->mutex_);
    ...
    if (found == impl_->hook_leases_.end() || callback_lease.generation != owner.scope->Generation() ||
        ...
        return Status(CABBIRD_STATUS_V1_NOT_FOUND, "hook callback lease is stale");
    }
    impl_->hook_leases_.erase(found);
    return Status(CABBIRD_STATUS_V1_OK);
}

CabbirdStatusV1 ScopedPlatformServices::ApplyPatch(
    const ScopedPluginServiceOwner& owner,
    const std::uintptr_t address,
    const CabbirdByteSpanV1 replacement,
    ...
    try {
        {
            ...
            if (conflict) return Status(CABBIRD_STATUS_V1_CONFLICT, "patch range conflicts with a tracked patch");
        }
        ...
        if (!original || original->size() != replacement.size) {
            return Status(CABBIRD_STATUS_V1_FAILED, "patch target could not be read");
        }
        ...
        if (!impl_->memory_services_.memory->PatchMemory(address, replacement.data, replacement.size)) {
            static_cast<void>(owner.scope->Release(state->token));
            return Status(CABBIRD_STATUS_V1_FAILED, "patch target could not be written");
        }
        state->applied = true;
        {
            std::scoped_lock lock(impl_->mutex_);
        ...
        *handle = {state->token, owner.scope->Generation()};
        ...
        return Status(CABBIRD_STATUS_V1_FAILED, "patch application failed");
    }
}

CabbirdStatusV1 ScopedPlatformServices::ReleasePatch(
    const ScopedPluginServiceOwner& owner, const CabbirdGenerationHandleV1 handle) noexcept {
    ...
    if (!Owns(state, owner, handle)) return Status(CABBIRD_STATUS_V1_NOT_FOUND, "patch handle is stale");
    return owner.scope->Release(handle.id)
        ? Status(CABBIRD_STATUS_V1_OK)
        : Status(CABBIRD_STATUS_V1_NOT_FOUND, "patch handle is stale");
}
