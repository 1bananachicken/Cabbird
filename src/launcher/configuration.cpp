...
constexpr std::wstring_view kGameExecutableName = L"AzurPromilia.exe";
constexpr std::wstring_view kLauncherExecutableName = L"launcher.exe";
constexpr std::wstring_view kLauncherExecutableNameGlobal = L"launcher.exe";

std::wstring_view LauncherExecutableName(const AzurPromiliaClient client) noexcept {
    return client == AzurPromiliaClient::Global
        ? kLauncherExecutableNameGlobal : kLauncherExecutableName;
}
...
bool LooksLikeAzurPromilia(const std::wstring_view value) {
    // The uninstall entry this title ships is DisplayName 蓝色星原：旅谣
    // (UCN below, see the file header for why) with DisplayIcon
    // D:\AzurPromilia\0.6.2.2\launcher.exe, and InstallLocation D:\AzurPromilia.
    const std::wstring folded = Fold(std::wstring(value));
    return folded.find(L"\u84dd\u8272\u661f\u539f") != std::wstring::npos ||
        folded.find(L"azurpromilia") != std::wstring::npos ||
        folded.find(L"launcher.exe") != std::wstring::npos;
}

void CollectAppPath(
    SystemLocations& result, const HKEY hive, const REGSAM registry_view,
    const AzurPromiliaClient client) {
    RegistryKey key;
...
void CollectUninstallLocations(
    ...
    for (DWORD index = 0;; ++index) {
        ...
        if ((!display_name || !LooksLikeAzurPromilia(*display_name)) &&
            (!display_icon || !LooksLikeAzurPromilia(*display_icon))) {
            continue;
        }
...
void AddNamedRoots(
    ...
    // AzurPromilia install layout.  The game
    // lives in `<install>\AzurPromilia_Game\` and the Electron launcher lives in a *version*
    // directory beside it (`<install>\0.6.2.2\launcher.exe`), not at the install root.  NTE's
    // UE-shaped `HTGame\Binaries\Win64` candidates had no counterpart here at all.
    constexpr std::array<std::wstring_view, 5> names{
        L"AzurPromilia", L"\u84dd\u8272\u661f\u539f\uff1a\u65c5\u8c23",
        L"Games\\AzurPromilia", L"Hotta\\AzurPromilia",
        L"Program Files\\AzurPromilia",
    };
    for (const auto name : names) roots.push_back(parent / name);
}

SystemLocations CollectSystemLocations(const AzurPromiliaClient client) {
...
std::optional<std::filesystem::path> FindGameDirectory(
    ...
    constexpr std::array<std::wstring_view, 5> candidates{
        L"", L"AzurPromilia_Game",
        L"Games\\AzurPromilia\\AzurPromilia_Game",
        L"Program Files\\AzurPromilia\\AzurPromilia_Game",
        L"AzurPromilia\\AzurPromilia_Game",
    };
    for (const auto& root : roots) {
        for (const auto relative : candidates) {
            const auto directory = relative.empty() ? root : root / relative;
            if (IsAzurPromiliaGameDirectory(directory)) return directory.lexically_normal();
        }
    }
    return std::nullopt;
}
...
std::optional<std::filesystem::path> FindLauncherExecutable(
    const std::vector<std::filesystem::path>& roots, const AzurPromiliaClient client) {
    // AzurPromilia install layout.  The game
    // lives in `<install>\AzurPromilia_Game\` and the Electron launcher lives in a *version*
    // directory beside it (`<install>\0.6.2.2\launcher.exe`), not at the install root.  NTE's
    // UE-shaped `HTGame\Binaries\Win64` candidates had no counterpart here at all.
    constexpr std::array<std::wstring_view, 7> parents{
        L"", L"Launcher", L"AzurPromilia", L"Games\\AzurPromilia",
        L"Program Files\\AzurPromilia", L"AzurPromilia_Game", L"0.6.2.2",
    };
    for (const auto& root : roots) {
        for (const auto parent : parents) {
            ...
            if (IsAzurPromiliaLauncherExecutable(executable, client)) {
...
std::filesystem::path LauncherConfigurationPath(
    ...
    try {
        return payload_root.empty()
            ? std::filesystem::path{}
            : payload_root.lexically_normal() / L"CabbirdLauncher.json";
    } catch (...) {
        return {};
    }
}
        ...
        if (schema_version == 1 && document.contains("proxy") &&
            ...
            result.configuration.selected_client =
                document.value("selectedClient", std::string{}) == "global"
                    ? AzurPromiliaClient::Global : AzurPromiliaClient::MainlandChina;
        } else {
            throw std::invalid_argument("unsupported launcher configuration shape");
        }
        result.loaded = true;
...
LauncherConfigurationSaveResult SaveLauncherConfiguration(
    ...
    try {
        ...
        const nlohmann::json document{
            {"schemaVersion", 2},
            {"selectedClient", configuration.selected_client == AzurPromiliaClient::Global
...
bool IsAzurPromiliaGameDirectory(const std::filesystem::path& directory) noexcept {
    try {
...
bool IsAzurPromiliaLauncherExecutable(
    const std::filesystem::path& executable, const AzurPromiliaClient client) noexcept {
    try {
...
LauncherClientConfiguration DiscoverLauncherConfiguration(
    ...
    if (IsAzurPromiliaGameDirectory(preferred.game_directory)) {
        result.game_directory = preferred.game_directory.lexically_normal();
    }
    if (IsAzurPromiliaLauncherExecutable(preferred.launcher_executable, options.client)) {
    ...
    for (const auto& executable : launcher_executables) {
        if (IsAzurPromiliaLauncherExecutable(executable, options.client)) {
