...
extern "C" void CabbirdRemoteBootstrapBegin();
extern "C" void CabbirdRemoteBootstrapEnd();
...
static_assert(offsetof(RemoteBootstrapContext, cabbird_start) == 56);
...
std::set<DWORD> SnapshotLauncherProcessIds(
    ...
    // The launcher process names, ADAPTED to this title.
    //
    // Upstream lists twelve names here -- NTELauncher/NTEGame/NTEBrowser/NTEWebBooster/NTEUpdate/
    // NTEErrRep plus the same six with a "Global" infix.  Copied across unchanged, none of them can
    // ever match a process on this title, so the snapshot below came back empty every time and
    // TerminateExistingLauncherProcesses (called at the mapping path, see below) silently did nothing
    // while still reporting success.  A dead branch that reports success is the worst shape a bug can
    // take, and nothing caught it: it compiles, it links, and it is only observable on a running game,
    // which this repository's rules forbid the agent from starting.
    //
    // What the replacement values are and where they came from: `config/AzurPromilia`'s launcher
    // executable is `launcher.exe`, which is not a guess -- it is the value the port already wrote into
    // src/launcher/configuration.cpp (kLauncherExecutableName and kLauncherExecutableNameGlobal, both
    // L"launcher.exe"), where the ("NTELauncher.exe", "launcher.exe") rules are recorded.  Those
    // rules were derived from configuration.cpp alone, which is precisely how this file came to be
    // missed.
    //
    // The two client variants coincide on this title, so upstream's twelve entries collapse to one.
    // Do NOT pad this list back out with invented names: which of the launcher suite's other processes
    // exist here (the equivalents of Game/Browser/WebBooster/Update/ErrRep) has NOT been measured, and
    // a guessed name would either do nothing -- the same silent bug again -- or terminate a process
    // nobody meant to touch.  Add an entry only with evidence from the game directory.
    //
    // The path check in the loop keeps this narrow: a candidate is terminated only if its executable
    // lives under `launcher_directory`, so a `launcher.exe` belonging to some unrelated product is not
    // affected.
    constexpr std::array<std::wstring_view, 1> launcher_names{L"launcher.exe"};
    std::set<DWORD> result;
    Handle snapshot(CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0));
    if (!snapshot) {
        error = GetLastError();
...
bool RemoteImageExportsCabbirdStart(
    ...
    for (DWORD rva : names) {
        ...
        if (ReadProcessMemory(
            ...
            std::string_view(name.data()) == CABBIRD_CORE_START_ENTRY) {
            return true;
        }
    }
    return false;
...
bool HasPrivateMappedCore(HANDLE process) {
    ...
    while (address < maximum) {
        ...
        if (region.State == MEM_COMMIT && region.Type == MEM_PRIVATE &&
            ...
            RemoteImageExportsCabbirdStart(process, base, region.RegionSize)) {
            return true;
        }
...
[[maybe_unused]] RemoteAllocation WriteRemoteWideString(HANDLE process, const std::filesystem::path& path) {
    const std::wstring value = path.wstring();
    const std::size_t bytes = (value.size() + 1) * sizeof(wchar_t);
...
std::filesystem::path ResolveOverlayIniPath(const std::filesystem::path& explicit_path,
    ...
    // The injector's own directory.  This is the anchor because the injector is
    // what the operator launches and what the build copies the ini next to -- not
    // because the mapping layer has any opinion about where config should live.
    //
    // GetModuleFileNameW(nullptr) resolves to the calling process' executable,
    // which for every front end of this library is the injector itself.
    std::wstring self(32768, L'\0');
    const DWORD self_length =
        ::GetModuleFileNameW(nullptr, self.data(), static_cast<DWORD>(self.size()));
    if (self_length != 0) {
        self.resize(self_length);
        std::filesystem::path beside_exe =
            std::filesystem::path(self).parent_path() / L"cabbird_overlay.ini";
        if (std::filesystem::exists(beside_exe, ini_ec)) {
            return beside_exe;
        }
    }

    // The image's own fallback location.  Checked so that a caller who deliberately
    // keeps the ini with the logs still gets it read, rather than being silently
    // ignored now that this helper exists.
    std::filesystem::path beside_logs = log_directory / L"cabbird_overlay.ini";
    if (!log_directory.empty() && std::filesystem::exists(beside_logs, ini_ec)) {
        return beside_logs;
    }

    // Nothing found: an empty path means the image uses its compiled-in defaults,
    // and it logs `present=0` when it does.  Returned rather than guessed at.
    return {};
}
    ...
    try {
        if (HasPrivateMappedCore(process.Get())) {
            return Failure(
                ...
                "an externally mapped Cabbird overlay is already present");
        }
        ...
        auto* headers = Headers(parsed->mapped);
        const auto start_rva = ExportRva(parsed->mapped, CABBIRD_CORE_START_ENTRY);
        if (headers == nullptr || !start_rva) {
            return Failure(
                ManualMapError::ImageInvalid, ERROR_PROC_NOT_FOUND,
                "core image does not export CabbirdStart");
        }
        ...
        const auto cxx_throw_rva = ExportRva(
            parsed->mapped, CABBIRD_CORE_MANUAL_MAP_CXX_THROW_ENTRY);
        if (!cxx_throw_rva) {
            return Failure(
                ManualMapError::ImageInvalid, ERROR_PROC_NOT_FOUND,
        ...
        if (!cxx_throw_override.applied) {
            /* REFUSED, exactly as upstream does.  Do not soften this back into a
             * "skip the bridge" warning.
             *
             * A /MT (static CRT) image resolves _CxxThrowException at link time, so
             * there is no import entry to redirect and the bridge cannot be
             * installed.  This project used to accept such an image with a
             * "C++ exception bridge skipped" note, justified by the untested claim
             * that a static-CRT image has "no cross-module unwinding problem
             * either".  That claim is false: inside a manually mapped /MT image
             * NO C++ exception is caught at all -- a bare
             * `try { throw 42; } catch (...) {}` reached
             * std::terminate -> abort -> __fastfail(FAST_FAIL_FATAL_APP_EXIT) and
             * took the whole host process down with 0xC0000409.  That is the
             * reported symptom: the game exits silently a few seconds after
             * injection, because the first throw anywhere in the runtime kills it.
             *
             * Upstream makes this configuration impossible.  Accepting it is what
             * turned a manifest-data diagnostic into a dead host process, so the
             * refusal is restored. */
            return Failure(
                ManualMapError::ImageInvalid, ERROR_PROC_NOT_FOUND,
        ...
        /* Both wide strings are allocated INSIDE the target process, because CabbirdStartInfo
         * stores pointers and the runtime dereferences them.  They must outlive the start
         * call, which is why they are RemoteAllocation locals here rather than being freed
         * after the struct is written.  This is upstream's arrangement verbatim; an earlier
         * version of this file inlined the paths into fixed-size arrays instead and asserted
         * in a comment that pointers could not be used. */
        ...
        const CabbirdStartInfo start_info{
            .struct_size = CABBIRD_START_INFO_V1_SIZE,
            .bootstrap_abi_version = CABBIRD_BOOTSTRAP_ABI_VERSION,
            .bootstrap_type = CABBIRD_BOOTSTRAP_TYPE_EXTERNAL,
            .flags = 0,
        ...
        if (remote_start_info.address == nullptr || WriteProcessMemory(
            ...
            return Failure(
                ManualMapError::WriteFailure, GetLastError(),
                "remote CabbirdStartInfo could not be written");
        }
        ...
        RemoteBootstrapContext context{
            ...
            .cabbird_start = remote_image_address + *start_rva,
        ...
        const auto* bootstrap_begin = reinterpret_cast<const std::byte*>(
            reinterpret_cast<const void*>(&CabbirdRemoteBootstrapBegin));
        const auto* bootstrap_end = reinterpret_cast<const std::byte*>(
            reinterpret_cast<const void*>(&CabbirdRemoteBootstrapEnd));
        ...
        if (context.bootstrap_error != ERROR_SUCCESS ||
            ...
            return Failure(
                ...
                "Cabbird overlay initialization failed in the target process",
        ...
        return {ManualMapError::None, ERROR_SUCCESS, ERROR_SUCCESS,
                remote_image_address, "Cabbird overlay attached"};
    } catch (...) {
        return Failure(
