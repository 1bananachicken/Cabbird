...
# Ported from Anomaly's tools/package_release.ps1, which is the packaging step its GitHub
# workflow runs on a tag.  The structure is upstream's on purpose -- validate the version
# against the configured project version, install each release component into its own
# stage, refuse to publish a Symbols archive that does not match the shipped PEs, zip
# deterministically, emit an SPDX SBOM per component, then verify every checksum -- and
# what changed is only the names: Anomaly -> Cabbird, the four components are this tree's
# (GameRuntime/SDK/Tools/Symbols), and the plugin PDB mapping points at
# Cabbird/plugins/<Package>/plugin.dll.
#
# The two checks worth keeping are the ones that fail loudly on a wrong release rather
# than shipping one: the project version in CMakeCache.txt must equal the version being
# packaged (so a tag cannot publish a binary built from a different version), and the
# Symbols archive must contain exactly one PDB per published PE (so "symbols are
# published" is a fact rather than a directory that happens to exist).
...
# InstallComponent is the CMake component name; Component is the SBOM/report name.  They
# are separate fields because GameRuntime is not called "Runtime" anywhere a user sees.
$packages = @(
    [pscustomobject]@{
        InstallComponent = 'GameRuntime'; Component = 'Runtime'; Slug = 'runtime'
        Name = "Cabbird-$Version-runtime"
    },
    [pscustomobject]@{
        InstallComponent = 'SDK'; Component = 'SDK'; Slug = 'sdk'
        Name = "Cabbird-$Version-sdk"
    },
    [pscustomobject]@{
        InstallComponent = 'Tools'; Component = 'Tools'; Slug = 'tools'
        Name = "Cabbird-$Version-tools"
    },
    [pscustomobject]@{
        InstallComponent = 'Symbols'; Component = 'Symbols'; Slug = 'symbols'
        Name = "Cabbird-$Version-symbols"
    }
)
...
# Each shipped PE must have exactly one corresponding linker PDB in the separately
# distributed Symbols component.
#
# The plugin branch is why this check is worth having at all: every plugin's binary is
# literally named plugin.dll (OUTPUT_NAME "plugin"), so the PDB name has to come from the
# package directory, and CMakeLists.txt installs them under that name with RENAME.  A
# plugin whose PDB was not renamed would show up here as an unexpected `plugin.pdb`.
    ...
    Get-ChildItem -LiteralPath $stages[$component] -Recurse -Force -File |
        ...
        ForEach-Object {
            ...
            $pdbName = if ($component -eq 'Runtime' -and
                $relative -match '(?i)^Cabbird/plugins/([^/]+)/plugin\.dll$') {
                "$($Matches[1]).pdb"
            } else {
                "$($_.BaseName).pdb"
            }
